...
const GOALS = [1.5, 2, 2.5, 3, 3.5]; // liters

export const WaterTracker = () => {
  const [ml, setMl] = useState(750); // in ml
  const [goal, setGoal] = useState(() => {
    const saved = localStorage.getItem("water-goal-ml");
    return saved ? parseFloat(saved) : 2; // liters
  });
    ...
    <div className="bg-card/80 backdrop-blur-sm border border-border rounded-2xl p-4 flex flex-col">
      ...
      <div className="flex items-center gap-2 mt-1">
        <button
          ...
          disabled={ml === 0}
          className={cn(
            "w-6 h-6 rounded-lg flex items-center justify-center transition-all border text-xs",
            ml === 0
